...
| `api_key` | 空 | 网关鉴权密钥；**空 = 不鉴权直接放行**（公网必须设置）。**主密钥，不受模型白名单限制** |
| `keys` | `[]` | 受限密钥列表（面板「白名单密钥」管理）。每项 `{key, name, allow[]}`，`allow` 为该密钥可调用的模型白名单；**为空 = 该密钥禁止调用任何模型**。详见「白名单密钥」章节 |
...
- **`api_key` 模式**（默认）：`api_key` 非空时面板要求输入一次密钥（浏览器 localStorage 记住）；为空则直接可用。
- **账密登录模式**：`web_auth.enabled` 为 `true` 时，面板先用用户名/密码登录换取会话 Cookie，网关 API 仍用 `api_key`。
...
| **成长任务队列** | 顶部统计（进行中/已结束、完成计数）+ 按账号分组的待办列表：任务名、进度（如 `1/1`）、状态（完成/失败/跳过）、结果说明。每行是可点击的待办项，点开查看详情 |
...
**窄屏适配（手机）**：任务中心的**两张宽表**都做了整表横向滑动：

- **成长任务队列**：5 列（账号 | 任务 | 进度 | 状态 | 结果说明），说明文字最长 40+ 字
  （如「当前不在 23:00–08:00 计数窗口，行为不计分；网关会在每日 23 点自动补足」）
- **开学季表**：账号 | 分享 | 桌面 | 对话×3 | 专家 | 认证 | 剩余抽奖，合计宽 ~560px+

两者在 390px 手机屏上都塞不下，会截断成只露开头。处理方式一致：

- **整表横向滚动**（`.qscroll` / `.swrap-scroll`）：所有行（含表头）同步平移，
  一次拖到底即可对齐看完每列完整内容
- **左右翻页按钮**（`‹` / `›`）：横向滚动手势在部分环境不可靠——触屏斜向拖动可能被判为纵向滚动，
  微信等 WebView 对嵌套横向滚动支持也参差。点按按钮不依赖手势识别，任何环境都能看全；
  按钮仅在内容确实溢出时显示，滑到头自动禁用

> 实现避坑：`overflow-y: visible` 与 `overflow-x: auto` 同用会被浏览器**强制把 `visible` 算成 `auto`**
> （CSS 规范），使容器纵向也独立滚动、干扰触摸手势，故显式写 `overflow-y: hidden`；
> `touch-action` 用 `pan-x` 而非 `pan-x pan-y`（后者在部分 WebView 里会让横向手势失效）。
...
除 `api_key` 外，面板还支持**用户名 + 密码**登录，适合把面板暴露给多人或不想共享 `api_key` 的场景。
**默认关闭**（`web_auth.enabled` 为 `false`）——老配置行为完全不变，面板仍走 `api_key`。
...
#    非交互（脚本化，注意会留在 shell 历史）：
./wb2api setpassword admin --password 'your-password'

# 3. 重启进程（用户库在启动时加载）
```

`setpassword` 还是一个**用户管理入口**：

```bash
./wb2api setpassword --list              # 列出已有用户（只显示用户名，不显示哈希）
./wb2api setpassword alice               # 新增用户 / 重置密码
...
| 字段 | 默认 | 说明 |
|---|---|---|
| `enabled` | `false` | 总开关。关闭时完全不生效，面板走 `api_key` |
| `users` | `{}` | 内联用户表（用户名 → 密码哈希）。一般留空，用 `users_file` 管理 |
| `users_file` | 空（仅内存） | 用户库落盘路径，**建议 `./data/webusers.json`**（`data/` 已在 `.gitignore`）。为空则重启后用户丢失 |
| `session_ttl` | `12h` | 会话有效期，Go duration 字符串。**固定过期，不做滑动续期** |
| `cookie_secure` | `false` | 会话 Cookie 是否带 `Secure`。HTTPS 部署应设 `true`；**纯 HTTP 部署必须 `false`**，否则浏览器不回传 Cookie、登录直接不可用 |
| `max_login_attempts` | `10` | 单 IP 在 `login_window` 内的最大失败次数，超出后拒绝 |
| `login_window` | `5m` | 失败计数滑动窗口 |

**安全设计**（明确记录，便于审计）

- 密码存储：**PBKDF2-SHA256** 加盐哈希，明文密码绝不落盘；`config.json` 与用户库分开存，
  避免密码哈希出现在"被人随手贴出来"的配置文件里
- 会话令牌：32 字节 `crypto/rand`，服务端**只存 SHA-256 摘要**（内存泄漏或日志误打印不直接泄令牌）；
  下发的 Cookie 设 `HttpOnly` + `SameSite=Lax`（`Secure` 由配置控制）
- 暴力破解：登录失败按 IP 计数限速，成功登录清零
- CSRF：所有状态变更请求需带 `X-CSRF-Token` 头，与 `wb2api_csrf` Cookie 双提交比对
  （见 `internal/panel/csrf.go`）
- **不做滑动续期**：面板是运维工具，重登成本低，缩短令牌有效窗口收益更大

**忘记密码 / 账号被锁**：直接用 `setpassword <用户名>` 重设（覆盖旧哈希）。
若完全无法进入，可在 `config.json` 里把 `web_auth.enabled` 改回 `false` 临时用 `api_key` 恢复，
改完记得立即处理并改回。

### 白名单密钥（受限 Key）

除主密钥 `api_key`（不受限）外，可在面板「白名单密钥」区域创建**多个受限密钥**，
每个密钥单独限定"只能调用哪些模型"，用于分发给他人的场景。

**配置字段**（`config.json` 的 `keys` 段，由面板自动读写）

| 字段 | 说明 |
|---|---|
| `key` | 密钥值（面板可一键随机生成 32 字节十六进制，形如 `sk-…`） |
| `name` | 备注名，便于识别用途 |
| `allow` | 允许的模型列表（字符串数组）。**支持国内 / 国际混合** |

**鉴权与模型校验**

- 请求头 `Authorization: Bearer <key>`：命中 `keys[]` 中某项即在网关侧放行（不走 `api_key`）
- 模型校验：只有 `allow` 中列出的模型可调用；**其它模型一律 `403 model_not_allowed`**
  （拦截在转发上游之前完成，不消耗账号额度）
- **`allow` 为空 = 该密钥禁止调用任何模型**（安全默认：避免"忘记勾选 = 全开"的越权）。
  面板会对未配置模型的密钥显示红色警告
- `GET /v1/models` 对受限密钥**只返回白名单内的模型**，客户端不会看到用不了的模型

**模型名格式（realm 前缀）**

模型 ID 统一带 realm 前缀，分别对应国内 / 国际账号池：

| 前缀 | 含义 | 示例 |
|---|---|---|
| `cn:` | 国内版（`*.codebuddy.cn`） | `cn:deepseek-v4.1-flash` |
| `global:` | 国际版（`*.workbuddy.ai`） | `global:gemini-3.5-flash` |
...
- **一键加载所有账号模型**：汇总**全部账号**（含多个 cn / global 账号）的模型目录，
  按「国内 / 国际」分组展示、**不去重**（同名模型在多个账号间重复出现属预期）。
  用于账号池目录分散时一次看全
- **手动添加模型名**：输入框可直接填任意模型名（含账号池目录未列出的），
  加前缀后并入白名单。手动项在列表中带「手动」标记，可随时取消勾选删除
- **增 / 删 / 改**：新建、改名、删除密钥，勾选或取消模型，保存即刻热生效（**无需重启**）

> **关于"上游可用但目录不列"的模型**：部分模型（如 `global:deepseek-v4.1-flash`）上游实际可调用，
> 但不在 `/v3/config` 的模型目录里。此类模型可**手动添加**进白名单——调用通、
> `GET /v1/models` 也会列出（带 `manual:true` 标识），仅缺少上游未下发的元数据
> （如 `context_length`）。

**配置持久化注意**：`config.json` 以**单文件 bind mount** 挂进容器（`./config.json:/app/config.json`），
存在两个部署约束：

1. 容器内无法对挂载点做 `rename`（原子替换）→ 保存时**回退为原地写入**（网关已处理）
2. **文件属主必须与容器运行 UID 一致**（默认 `10001`），否则保存报 `permission denied`：
