...
**窄屏适配（手机）**：成长任务队列是 5 列宽表（列至少需 ~430px），塞不进 390px 手机屏，
说明文字（如「当前不在 23:00–08:00 计数窗口，行为不计分；网关会在每日 23 点自动补足」共 43 字）
会被截断只露出开头。处理方式：

- **整表横向滚动**（`.qscroll > .qtable`）：所有行同步平移，一次拖到底即可对齐看完每行完整说明
  （而不是每行各自滚动、看一行滑一次）
